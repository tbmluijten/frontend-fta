...
$items   = fta_landing_faq_items();
$faq_uid = function_exists('wp_unique_id') ? wp_unique_id('faq-') : 'faq-' . wp_rand(1000, 9999);
?>
<section class="bg-white" aria-labelledby="faq-heading" data-faq-root>
	<div class="mx-auto w-full max-w-[1200px] px-4 py-[40px] md:py-[70px]">
		<div class="mx-auto max-w-[720px] text-center">
			<h2 id="faq-heading" class="text-[30px] font-semibold leading-none text-[#0E1629]">
				<?php esc_html_e('Frequently Asked Questions', 'fta-landing'); ?>
			</h2>
			<p class="mt-4 text-[16px] font-normal leading-6 text-[#465469] lg:text-[18px] lg:leading-none">
				<?php esc_html_e('Common questions about pursuing a claim and getting support.', 'fta-landing'); ?>
			</p>
		</div>
			...
			<?php foreach ($items as $index => $item) : ?>
				<li>
					<div class="rounded-xl border border-[#E2E8F0] bg-white px-5 py-4 lg:px-6 lg:py-5">
						<h3 class="m-0 text-[18px] font-medium leading-none text-[#0A3182]">
							<button
							...
							>
								...
								<span class="faq-chevron mt-0.5 shrink-0 text-[#0A3182] transition-transform duration-300 ease-out <?php echo 0 === $index ? 'rotate-180' : 'rotate-0'; ?>" aria-hidden="true">
						...
						>
							<div class="min-h-0 overflow-hidden">
								<p class="pt-3 text-[16px] font-normal leading-6 text-[#465469] lg:pt-4 lg:leading-[27px]">
